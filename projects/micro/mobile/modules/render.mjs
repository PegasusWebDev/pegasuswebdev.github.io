...
function renderTitle(world){
	...
	$('#title').text(worldcopy.replaceAll(' ','&nbsp;'));
}
function renderBody(world){
  ...
  var el = $('#gameworld').get(0);
  el.scroll({
    left: 32.5*(world.player.pos[0])-(el.clientWidth/2),
    top: 32*(world.player.pos[1]+1)-el.clientHeight/2,
    behavior: "smooth"
  });
}
    ...
    var hasNextLevel = !(workshop.level+1 == workshop.LEVELS.length);
		if(hasNextLevel){ //If there is a next level
			...
			if(meetsAllRequirements){
				$('#workshops').append('<p><a href="#" onclick="world.spaceAction(world.player.pos[0], world.player.pos[1])"> ◆ </a></p>');
			}
		}
...
function renderWanderer(world){
	...
	if(world.wanderer.exists && world.wanderer.pos[0] == world.player.pos[0] && world.wanderer.pos[1] == world.player.pos[1]){
		...
		if(world.player.inventory['◊'].quantity>=world.wanderer.hireCost){
			$('#wanderer').append('<br><a href="#" onclick="world.spaceAction(world.player.pos[0], world.player.pos[1])"> ▣ </a>');
		}
	}
}
function renderMarket(world){
	...
	if(world.gameworld[world.player.pos[1]][world.player.pos[0]] == '▦'){
		...
		for(var i in world.world.marketplace.trades){
			...
			if(hasAllItems){
				$('#market').append('<a href="#" onclick="world.doMarketTrade('+i+')">'+i+': '+costString+'-> '+returnString+'</a>');
